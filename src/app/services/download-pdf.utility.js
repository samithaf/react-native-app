...
export const downloadPDF = async (url, fileName): Promise<void> => {
  const mime = 'application/pdf';
  const path = `${RNFetchBlob.fs.dirs.DownloadDir}/${encodeURIComponent(
    fileName,
  )}.pdf`;
  ...
  if (res.respInfo.status === 200) {
    const isValidPDF = Object.entries(res.respInfo.headers).some(
      ([key, value]) => /content-type/gi.test(key) && value === mime,
    );
    if (!isValidPDF) {
